...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  case gMcpsDataInd_c:
    ...
    break; */
	  ...
	  /* Print the data requested for phase 1: Short address, LQI and payload size */
	  Serial_Print(interfaceId, "\n\r--- Received packet ---\n\r", gAllowToBlock_d);

      Serial_Print(interfaceId, "MAC address (Short): 0x", gAllowToBlock_d);
      Serial_PrintHex(interfaceId, (uint8_t *)&pMsgIn->msgData.dataInd.srcAddr, 2, gPrtHexNoFormat_c);

      Serial_Print(interfaceId, "\n\rLQI: ", gAllowToBlock_d);
      Serial_PrintDec(interfaceId, (uint32_t)pMsgIn->msgData.dataInd.mpduLinkQuality);

      Serial_Print(interfaceId, "\n\rPayload size: ", gAllowToBlock_d);
      Serial_PrintDec(interfaceId, (uint32_t)pMsgIn->msgData.dataInd.msduLength);
      ...
      /* LEDS control based on the received payload */
      if(pMsgIn->msgData.dataInd.msduLength >= 10)
      {
    	  uint8_t *payload = pMsgIn->msgData.dataInd.pMsdu;

    	  /* Verify that the packet is the type "Counter: x" by checking the string format */
    	  if(payload[0] == 'C' && payload[1] == 'o' && payload[8] == ' ')
          {
        	  uint8_t counterVal = payload[9] - '0'; /* Convert the ASCII character to its integer value */

              LED_TurnOffAllLeds();
              switch(counterVal)
              {
              	  case 0:
              		  Led3On(); /* Turn on Green LED */
                      break;
                  case 1:
                      Led2On(); /* Turn on Red LED */
                      break;
                  case 2:
                      Led4On(); /* Turn on Blue LED */
                      break;
                  case 3:
                	  Led2On(); /* Turn on Magenta LED (Red + Blue) */
                	  Led4On();
                      break;
              }
          }
