...
/* External reference to the custom transmission routine */
extern void App_TransmitCustomData(uint8_t *myData, uint8_t length);
                ...
                {   
                    ...
                    {          
                        ...
                        { 
                            ...
                            MyTaskTimer_Start(); /*Start task timer*/
...
/* Custom function to transmit data based on "App_TransmitUartData" */
void App_TransmitCustomData(uint8_t *myData, uint8_t length)
{
    nwkToMcpsMessage_t *mpMyPacket;
    ...
    /* Request an official memory block from the RTOS large enough to hold the MAC headers plus the payload (10 bytes). */
    mpMyPacket = MSG_Alloc(sizeof(nwkToMcpsMessage_t) + gMaxPHYPacketSize_c);

    if(mpMyPacket != NULL)
    {
        mpMyPacket->msgType = gMcpsDataReq_c; // Packet type: Define this as an MCPS Data Request
        mpMyPacket->msgData.dataReq.pMsdu = (uint8_t*)(&mpMyPacket->msgData.dataReq.pMsdu) + // Set the pointer to the payload section of the allocated memory
                                          sizeof(mpMyPacket->msgData.dataReq.pMsdu);

        /* Copy "Counter: 0" string directly into the MAC Service Data Unit (MSDU) payload buffer.*/
        FLib_MemCpy(mpMyPacket->msgData.dataReq.pMsdu, myData, length);

        /*Copy the network topology data (saved during network association) into the packets header. */
        FLib_MemCpy(&mpMyPacket->msgData.dataReq.dstAddr, &mCoordInfo.coordAddress, 8);
        FLib_MemCpy(&mpMyPacket->msgData.dataReq.srcAddr, &maMyAddress, 8);
        FLib_MemCpy(&mpMyPacket->msgData.dataReq.dstPanId, &mCoordInfo.coordPanId, 2);
        FLib_MemCpy(&mpMyPacket->msgData.dataReq.srcPanId, &mCoordInfo.coordPanId, 2);

        /* Configure addressing modes and payload size */
        mpMyPacket->msgData.dataReq.dstAddrMode = mCoordInfo.coordAddrMode;
        mpMyPacket->msgData.dataReq.srcAddrMode = mAddrMode;
        mpMyPacket->msgData.dataReq.msduLength = length; // 10 bytes
        mpMyPacket->msgData.dataReq.txOptions = gMacTxOptionsAck_c;
        mpMyPacket->msgData.dataReq.msduHandle = mMsduHandle++;
        mpMyPacket->msgData.dataReq.securityLevel = gMacSecurityNone_c;

        /* Push the full constructed packet into the MAC Layer's Service Access Point (SAP) to transmit the RF signal. */
        (void)NWK_MCPS_SapHandler(mpMyPacket, macInstance);
    }
}
